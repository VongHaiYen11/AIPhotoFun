  ...
  return (
    <div className="w-full max-w-7xl px-6 md:px-12 py-12 flex flex-col items-center text-white">
      <div className="w-full flex justify-between mb-8">
        {status ? <BackToTools /> : <GoBackTools onClick={() => setStatus(true)} />}
        <LanguageSwitcher />
      </div>
        ...
        <div className="flex flex-col items-center content-center w-full">
          <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="w-full md:col-span-1">
              <div className="bg-white/[0.04] backdrop-blur-xl border border-white/50 rounded-3xl p-8 mb-8 h-full">
                ...
                <div className="mt-6 flex flex-col gap-3">
                  <h4 className="font-semibold">{t('productMockupGenerator.aiGraphicDesigner')}</h4>
                  <textarea
                    value={AIDesignerPrompt} 
                    onChange={(e) => setAIDesignerPrompt(e.target.value)}
                    rows={3}
                    className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/15 text-white placeholder-white/40 resize-none focus:outline-none focus:border-white"
            ...
            <div 
              ...
              className={`rounded-2xl border border-white/15 bg-white/5 backdrop-blur-xl h-fit px-6 py-6 flex flex-col gap-4 items-start w-full md:col-span-1 transition-opacity ${!currentImage ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}
            >
                <h2 className="text-xl font-bold">{t('productMockupGenerator.designSettings')}</h2>
                <div className="rounded-xl border border-white/20 bg-white/5 backdrop-blur-xl h-fit px-4 py-4 flex flex-col items-start w-full">
                  <h3 className="font-bold mb-4">{t('productMockupGenerator.apparelDetails')}</h3>
                  <div className="inline-flex bg-white/5 rounded-xl p-1 border border-white/20 w-full mb-4">
                    <button onClick={() => setMode('Generate')} className={`flex-1 px-4 py-2 rounded-lg font-semibold transition ${mode === 'Generate' ? 'bg-white text-black' : 'text-white/70 hover:bg-white/10'}`}>
                      {t('productMockupGenerator.generateMockups')}
                    </button>
                    <button onClick={() => setMode('Upload')} className={`flex-1 px-4 py-2 rounded-lg font-semibold transition ${mode === 'Upload' ? 'bg-white text-black' : 'text-white/70 hover:bg-white/10'}`}>
                  ...
                  {mode === 'Upload' ? (
                    <div className="w-full mt-2 p-6 rounded-2xl bg-white/5 text-white/80 border border-white/10">
                      ...
                      <SideImageLoader value={uploadedMockup} onChange={setuploadedMockup} onRemove={() => setuploadedMockup(undefined)} fullWidth className="h-64" />
                    </div>
                  ) : (
                    <div className="mt-2 flex flex-col w-full">
                      <h4 className="font-semibold text-white mb-3">{t('productMockupGenerator.apparelStyle')}</h4>
                      <textarea
                        value={styleDescPrompt} onChange={(e) => setStyleDescPrompt(e.target.value)} rows={4}
                        className="w-full px-4 py-3 mb-2 rounded-xl bg-white/5 border border-white/15 text-white placeholder-white/40 resize-none focus:outline-none focus:border-white"
                        placeholder={t('productMockupGenerator.apparelStylePlaceholder')}
                      />
                      <p className="text-white/40 mb-3 text-sm">{t('productMockupGenerator.apparelStyleDesc')}</p>
                      <div className="w-full mt-2">
                        <h4 className="font-semibold text-white mb-2">{t('productMockupGenerator.mockupStyle')}</h4>
                        <SingleSelectList feature="productMockupGenerator" keys={mockupStyle} value={selectedMockup} onChange={setSelectedMockup} />
                      </div>
                    </div>
                  )}
                </div>

                <div className={`rounded-xl border border-white/20 bg-white/5 backdrop-blur-xl h-fit px-4 py-4 flex flex-col gap-4 w-full transition-opacity ${mode === 'Upload' ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                  <div>
                    <h3 className="font-bold mb-1">{t('productMockupGenerator.colorwayGenerator')}</h3>
                    <p className="text-white/40 text-sm">{t('productMockupGenerator.colorwayGeneratorDesc')}</p>
                  </div>
                  <div className="flex gap-2 w-full">
                    <input value={hexInput} onChange={(e) => setHexInput(e.target.value.toUpperCase())} placeholder="#FFFFFF" className="flex-1 px-4 py-3 rounded-lg bg-white/10 border border-white/20 text-white placeholder-white/40 focus:outline-none focus:border-white/40" />
                    <button disabled={!isValidHex(hexInput)} onClick={() => { if (!colorways.includes(hexInput)) setColorways(prev => [...prev, hexInput]); setHexInput('#'); }} className="px-6 rounded-lg bg-white/20 text-white font-semibold hover:bg-white/30 transition disabled:opacity-40">{t('productMockupGenerator.add')}</button>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {colorways.map(hex => (
                      <div key={hex} className="flex items-center gap-2 px-3 py-2 rounded-full bg-white/10 border border-white/20">
                        <div className="w-5 h-5 rounded-full border border-white/30" style={{ backgroundColor: hex }} />
                        <span className="text-sm font-medium">{hex}</span>
                        <button onClick={() => setColorways(prev => prev.filter(c => c !== hex))} className="ml-1 text-white/40 hover:text-white">×</button>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="w-fit mt-8">
            <button
              className="px-8 py-4 bg-white text-black rounded-xl font-bold text-lg hover:bg-white/90 disabled:opacity-50 shadow-xl flex items-center gap-3"
              disabled={!currentImage || (mode === 'Generate' && colorways.length === 0) || (mode === 'Upload' && !uploadedMockup)}
              onClick={handleGenerateMockups}
            >
               {t('productMockupGenerator.generateColorways', { count: mode === 'Upload' ? 1 : colorways.length })}
            </button>
          </div>
        </div>
      ) : (
