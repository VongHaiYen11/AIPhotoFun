...
export const ProductSceneGenerator: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
        for (const angleKey of selectedCameraAngle) {
            ...
            const readableName = t(`productSceneGenerator.${angleKey}`);
  ...
  return (
    <div className="w-full max-w-7xl px-6 md:px-12 py-12 flex flex-col items-center text-white">
      ...
      >
        <h1 className="text-5xl md:text-7xl font-black tracking-tighter mb-6">{t('app.productSceneGeneratorTitle')}</h1>
        <p className="text-white/60 text-lg md:text-xl max-w-3xl mx-auto">{t('productSceneGenerator.subtitle')}</p>
      </motion.div>
      ...
      {status ? (
          <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="w-full md:col-span-1">
              <div className="bg-white/[0.04] backdrop-blur-xl border border-white/50 rounded-3xl p-8 mb-8 h-full">
                <h2 className="text-xl font-bold mb-6 text-center">{t('productSceneGenerator.step1Title')}</h2>
                <ImageUpload value={currentImage} height="h-96" onChange={setCurrentImage} onRemove={() => setCurrentImage(undefined)} />
              </div>
            </div>

            <div className={`rounded-2xl border border-white/15 bg-white/5 backdrop-blur-xl h-fit px-6 py-6 flex flex-col items-start w-full md:col-span-2 ${!currentImage ? 'opacity-50 pointer-events-none' : ''}`}>
                <h2 className="text-xl font-bold mb-4 text-center">{t('productSceneGenerator.step2Title')}</h2>
                <p className="text-sm text-white/50 mb-4">{t('productSceneGenerator.step2Desc')}</p>
                <div className="w-full mb-8">
                    <MultiSelectList feature="productSceneGenerator" keys={prefixedAngles} value={selectedCameraAngle} onChange={setSelectedCameraAngle} />
                </div>
              <div className="flex w-full items-center justify-between mt-auto">
                ...
                <button className="px-6 py-3 bg-white text-black rounded-lg font-semibold" disabled={selectedCameraAngle.length === 0} onClick={handleGenerate}>
          ...
          <div className="flex flex-col gap-6 items-center w-full max-w-6xl">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full mb-16">
              {generatedResults.map((res, idx) => <ImageResultHolder key={idx} imageUrl={res.url} name={res.name} showRegenerate={false} />)}
              {isGenerating && (
